...
# ======================================================================
# 2) YOUR PIPELINE FUNCTIONS (as given)
# ======================================================================
...
def evaluate_model(name, model, X_tr, y_tr, X_val, y_val):
    ...
    if name == "Neural Network":
        ...
        # most models accept sample_weight; for safety, fallback if not supported
        try:
...
def train_pipeline(jst_path: str):
    ...
    # Test probs for ALL models (useful for dashboard switching)
    test_probs_by_model = {}
    for name, clf in fitted_models.items():
        test_probs_by_model[name] = clf.predict_proba(Xs_test)[:, 1]

    # For full-timeline scoring in dashboard:
    # Build scaled X for entire df_target (post-clean + target-aligned rows)
    ...
    return {
        ...
        "scaled": {"Xs_test": Xs_test, "y_test": y_test.to_numpy(), "X_full_scaled": X_full_scaled},
...
def run_upload_predict_sidebar(selected_model, scaler, threshold, base_features, missing_features, all_features):
    ...
    # Engineer + clean (adds missing flags)
...
Xs_test = bundle["scaled"]["Xs_test"]
y_test = bundle["scaled"]["y_test"]
X_full_scaled = bundle["scaled"]["X_full_scaled"]
...
# ======================================================================
# 6) SIDEBAR CONTROLS (model + filters)
# ======================================================================

st.sidebar.header("🎛️ Dashboard Controls")

model_choice = st.sidebar.selectbox(
    "Select model (validation-trained)",
...
threshold = st.sidebar.slider(
    ...
    min_value=0.05,
    max_value=0.90,
    value=float(best_thresh if model_choice == best_name else best_thresh),
    step=0.01,
    key="risk_threshold"
)
...
# We already have X_full_scaled aligned row-for-row with df_target
# So we need the matching subset indices
risk_idx = risk_df.index.to_numpy()
risk_scaled = X_full_scaled[np.searchsorted(df_target.index.to_numpy(), risk_idx)]
...
# -----------------------------------------------------------------------------
# TAB 1: Risk timeline (Streamlit-native chart)
with tab1:
...
# -----------------------------------------------------------------------------
# TAB 2: Model results (validation + test)
with tab2:
    st.header("Model comparison (validation)", divider="gray")
    st.dataframe(results_df, use_container_width=True)

    st.write("")
    st.subheader("Selected model", divider="gray")
    st.markdown(f"**Current model:** `{model_choice}`  \n**Threshold:** `{threshold:.2f}`")

    # Test metrics for current model
...
# -----------------------------------------------------------------------------
# TAB 4: SHAP (fixed) – on demand
with tab4:
    st.header("SHAP explainability", divider="gray")
    st.caption("Compute SHAP for the current model. For speed, we sample rows.")
    ...
    if st.button("Compute SHAP", type="primary"):
        with st.spinner("Computing SHAP..."):
            # Build SHAP input from TEST set (scaled), sample
            feature_names = all_features
            X_test_shap = pd.DataFrame(Xs_test, columns=feature_names)

            # sample
            Xs = X_test_shap.sample(n=min(sample_n, len(X_test_shap)), random_state=42)
    ...
    st.caption(
        "Note: SHAP can be heavier for some models (e.g., SVM/MLP). "
        "Use smaller samples when needed."
    )
...
with tab6:
    ...
    st.caption("This is the cleaned + engineered dataset used for modelling (includes missing flags + target).")
